    ...
    <main>
        ...
        <div className="container py-14 md:py-20 lg:py-24">
          <div className="mx-auto max-w-4xl text-center">
            <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-blue-100 bg-blue-50 px-3.5 py-2 text-xs font-bold text-blue-700 sm:text-sm">
              <span className="grid size-5 place-items-center rounded-full bg-white"><Icon className="size-3.5"><path d="m12 3 1.8 5.2L19 10l-5.2 1.8L12 17l-1.8-5.2L5 10l5.2-1.8L12 3Z" /></Icon></span>
              Daha akıllı alışveriş, daha doğru fiyat
            </div>
            <h1 className="text-4xl font-extrabold leading-[1.08] tracking-[-.045em] text-slate-950 sm:text-5xl md:text-6xl">Doğru ürünü,<br className="hidden sm:block" /> <span className="text-blue-600">doğru fiyata</span> bul.</h1>
            <p className="mx-auto mt-5 max-w-2xl text-base leading-7 text-slate-600 sm:text-lg">Güncel fiyatları, ürünleri ve FiyatPilot puanlarını tek yerde karşılaştır. Bütçene ve kullanım amacına göre daha hızlı karar ver.</p>

            <form action="/ara" className="mx-auto mt-8 flex max-w-2xl flex-col gap-2 rounded-2xl border border-slate-200 bg-white p-2 shadow-[0_12px_35px_rgba(15,23,42,.08)] sm:flex-row" role="search">
              <div className="relative flex min-w-0 flex-1 items-center">
                <Icon className="absolute left-3 size-5 text-slate-400"><circle cx="11" cy="11" r="7" /><path d="m20 20-4-4" /></Icon>
                <input name="q" aria-label="Aramak istediğiniz ürünü yazın" className="h-11 w-full min-w-0 bg-transparent pl-11 pr-3 text-sm outline-none" placeholder="Laptop, telefon, robot süpürge..." />
              </div>
            ...
            <div className="mt-5 flex flex-wrap items-center justify-center gap-2 text-sm">
              <span className="mr-1 text-slate-400">Popüler aramalar</span>
              {['Öğrenci laptop', 'En iyi laptop', 'Gaming laptop', 'Robot süpürge', 'Tablet'].map((x) => <a href={'/ara?q=' + encodeURIComponent(x)} className="rounded-full border border-slate-200 bg-white px-3 py-1.5 font-medium text-slate-600 transition hover:border-blue-200 hover:bg-blue-50 hover:text-blue-700" key={x}>{x}</a>)}
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="container py-12 md:py-16">
        <div className="mb-6 flex items-end justify-between gap-4">
          <div><p className="text-sm font-bold text-blue-600">Bugünün seçimi</p><h2 className="mt-1 text-2xl font-extrabold tracking-tight text-slate-950 md:text-3xl">Fiyat/performans seçimleri</h2><p className="mt-1 text-sm text-slate-500">Öne çıkan ürünleri hızlıca keşfet.</p></div>
      ...
      <section className="border-y border-slate-200 bg-white">
        <div className="container grid gap-px bg-slate-200 md:grid-cols-3">
          ...
          <Feature icon={<Icon><path d="m12 3 1.8 5.2L19 10l-5.2 1.8L12 17l-1.8-5.2L5 10l5.2-1.8L12 3Z" /></Icon>} title="AI karar desteği" text="Bütçene ve kullanımına göre seçenekleri daha kolay değerlendir." />
  ...
  return <article className="surface surface-hover overflow-hidden">
    <a href={'/urun/' + p.slug} className="block">
      <div className="relative aspect-[4/3] overflow-hidden bg-slate-100">
        ...
        <span className="absolute left-3 top-3 rounded-full bg-white/95 px-2.5 py-1 text-[11px] font-extrabold text-violet-700 shadow-sm">AI {p.aiScore}</span>
      </div>
    </a>
...
function Feature({ icon, title, text }: { icon: React.ReactNode; title: string; text: string }) {
  return <div className="bg-white p-6 md:p-8"><div className="mb-4 grid size-10 place-items-center rounded-xl bg-blue-50 text-blue-600">{icon}</div><h3 className="font-bold text-slate-950">{title}</h3><p className="mt-1.5 text-sm leading-6 text-slate-500">{text}</p></div>;
}
